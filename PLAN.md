...
M2.5 interface (next): replace the inline-styled screens with a domain-native interface (docs/adr/0009, docs/design-brief.md). A collapsible left rail of the six steps over a light content area; the Lines table and the graphical genotype become one selection model, so sorting or filtering either reorders both, with a single action bar reporting total, visible and selected lines; the canvas is surrounded by its context (labels in a fixed left gutter, overview below) instead of controls floating over data; design tokens for a hueless neutral ramp, IBM Plex Sans and Mono self-hosted, 32 px rows and 13 px interface type with density as a user control; React Aria for table, menu, dialog and form behaviour; and a texture per genotype class alongside its colour. Acceptance: the genotype view is legible printed in black and white, which it is not today, because recurrent blue and donor vermilion separate by only 1.34:1 in luminance; every class swatch and component boundary meets the 3:1 of WCAG 1.4.11, which heterozygous yellow, missing white and uninformative grey currently fail; the table is operable by keyboard under the W3C APG grid pattern with one tab stop into the grid; sorting the Lines table reorders the canvas and the reverse; and no literal colour or dimension appears in a component file.
