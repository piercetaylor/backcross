...
Genotypes as VCF, HapMap or wide CSV; samples.csv with exactly one `recurrent_parent` and one `donor_parent`; optional markers.csv with cM positions. The contract is specified in docs/data-formats.md and is shared with the sibling progeny-selector project so files move between the two tools unchanged.
